  ...
  VSTScrollbar(
    ...
    div(class="w100% overflow-hidden!")
      .vst-field-editor(
        ...
        :class=`{
          'b-solid b-1px b-transparent': !disabled,
        }`
        ref="vstFieldEditor"
...
@VST export default class TextField extends FieldComponent {
  ...
  mounted() {
    ...
    this.$el?.addEventListener?.('wheel', scrollListener, { passive: false })
    this.hookWhenComponentDestroy(() => this.$el?.removeEventListener?.('wheel', scrollListener))
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          // Лимитируем максимальную ширину
          let width = img.width
          let height = img.height

          if (width > this.compressedPasteImageWidthLimit) {
            height = (this.compressedPasteImageWidthLimit / width) * height
            width = this.compressedPasteImageWidthLimit
          }

          if (height > this.compressedPasteImageHeightLimit) {
            width = (this.compressedPasteImageHeightLimit / height) * width
            height = this.compressedPasteImageHeightLimit
          }
          canvas.width = width
...
.vst-field-editor
  ...
  .tiptap.ProseMirror
    ...
    @apply my1px outline-none! border-none! min-h120px
