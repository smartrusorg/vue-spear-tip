...
.file-item(
  ...
  @dragend="$parent.isDragStarted = false"
    ...
    .file-container(
      ...
      .file-content(
        ...
        VSTButton(
          size="md"
          theme="empty"
          @clickTap="$emit('view', fileName)"
        )
          div(
            class=`absolute right--15px top-0 py0 text-neutral-600! uppercase px8px fw-bold fs-0.7rem rounded-xl
            bg-amber-100 z1`
          ) {{ file.ext }}
          img(
            v-if="fileType == 'image'"
            :src="file.uri"
            class="max-w52px! max-h52px! rd-lg!"
          )
          svg(
...
<script lang="ts">
import {Prop, BaseComponent, Component} from '../../../../core'
...
export default class FileItem extends BaseComponent {
  ...
  @Prop(Boolean) readonly isImage: boolean = false
  ...
  startDrag(e: any) {
    ...
    this.$parent.isDragStarted = true
  ...
  onDrop(e: any) {
    ...
    let from: any = e.dataTransfer.getData('itemID')
    const to = {
      name: names[0],
      hash: names[1] ? names[1] : null,
    }
    if (typeof from == 'string' && from && from != JSON.stringify(to)) {
      from = JSON.parse(from)
      if (this.$parent?.files?.some?.(v => v.hash == from.hash)) { // Защита от сортировки между разными полями
...
.progress-bar
  @apply bg-emerald
  width: 50%
  /* Измените этот процент для отображения прогресса */
  height: 20px
  text-align: center
  line-height: 30px
  color: white
