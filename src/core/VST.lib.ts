...
VSTLib.onVariableCreated = (globalVariableString: string, onCreateCallback: () => any, timeoutInSeconds = 30) => {
  const resolveVariable = (path: string): boolean => {
    ...
    // Если путь начинается с 'window', пропускаем первый элемент, иначе идем с нуля
    ...
    // Проверяем именно значение (как делал eval), а не просто наличие ключа
    return typeof current !== 'undefined'
  }
  
  // Быстрая проверка: если переменная УЖЕ создана, выполняем сразу
  try {
    ...
    // Игнорируем возможные ошибки доступа на старте
  }
  
  // Если переменной еще нет — запускаем интервал ожидания
